...
public final class P256SignerPlugin: NSObject {
    ...
    public func createCredential(params: CreateCredentialParams, completion: @escaping (Result<CredentialResult, Error>) -> Void) {
        do {
            ...
            let delegate = RegistrationDelegate { result in completion(result) }
    ...
    public func getCredential(params: GetCredentialParams, completion: @escaping (Result<AssertionResult, Error>) -> Void) {
        do {
            ...
            let controller = ASAuthorizationController(authorizationRequests: [request])
            let delegate = AssertionDelegate { result in completion(result) }
            controller.delegate = delegate
            controller.presentationContextProvider = DefaultPresentationContext.shared
            objc_setAssociatedObject(controller, &AssociationKey.delegateKey, delegate, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            controller.performRequests()
...
private final class RegistrationDelegate: NSObject, ASAuthorizationControllerDelegate {
    ...
    init(completion: @escaping (Result<CredentialResult, Error>) -> Void) { self.completion = completion }
...
private final class AssertionDelegate: NSObject, ASAuthorizationControllerDelegate {
    ...
    init(completion: @escaping (Result<AssertionResult, Error>) -> Void) { self.completion = completion }
